...
function proData(data) {
  ...
  // 调整ie中的img左外间距和上外间距
  if (type == 'ie' || type == 11) {
    var imgs = document.querySelectorAll('#message ul img');
    for (var t = 1; t < imgs.length - 1; t++) {
      imgs[t].style.marginLeft = '30px';
      imgs[t].style.marginTop = '4px';
    }
  }
  ...
  if (pc_app == 1) {
    ...
    // pc下半部分直线
    line.style.height = 94 + 61.66 * (we - 2) + "px";
    //状态图标
    img.src = "img/运输状态_" + sta + "_pc.png";
    img.style.visibility = 'visible';
    // pc地图
    gps.src = "https://lmsqas.whchem.com/myscm/GPSGate?toId=" + getData[0].no;
  }
